# Squidler Quality Checks (`run-remote`)
...
It targets a site that Squidler's cloud workers can reach on their own (production, staging, a public preview deploy) via the Squidler GitHub integration. To run test cases against a URL that is only reachable from the runner — a `localhost` dev server, a private preview — use the [`run-local`](../run-local) action instead.
